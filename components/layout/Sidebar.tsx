...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  const handleToggleSidebar = () => {
    playSound(isOpen ? SOUNDS.TOGGLE_OFF : SOUNDS.TOGGLE_ON);
    setIsOpen(!isOpen);
  };

  return (
    <aside
      className={`relative flex flex-col bg-white dark:bg-gray-800 shadow-xl h-full transition-width duration-300 ease-in-out overflow-hidden ${
